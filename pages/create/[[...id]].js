...
const ProductsManager = () => {
    ...
    const handleUploadInput = e => {
        ...
        if(files.length === 0) 
        return dispatch({type: 'NOTIFY', payload: {error: 'Podane pliki nie istnieją.'}})

        files.forEach(file => {
            if(file.size > 1024 * 1024)
            return err = 'Plik nie może przekraczać 1MB pojemności'

            if(file.type !== 'image/jpeg' && file.type !== 'image/png')
            return err = 'Rozszrzenie pliku jest nieodpowiednie. Poprawne rozszerzenia to: jpeg, png.'
        ...
        return dispatch({type: 'NOTIFY', payload: {error: 'Możesz wybrać maksymalnie 5 zdjęć.'}})
    ...
    const handleSubmit = async(e) => {
        ...
        return dispatch({type: 'NOTIFY', payload: {error: 'Uwierzytelnianie jest nieprawidłowe.'}})

        if(!title || !price || !inStock || !description || !content || category === 'all' || images.length === 0)
        return dispatch({type: 'NOTIFY', payload: {error: 'Wypełnij wszystkie pola.'}})
